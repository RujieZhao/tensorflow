...
def new_op(weight,x_data,grad=None):
	rnd_name = 'PyFuncGrad' + str(np.random.randint(0, 1E+8))
	tf.RegisterGradient(rnd_name)(grad)	
	with tf.get_default_graph().gradient_override_map({"PyFunc":rnd_name}):
		output = tf.py_func(np_func,inp=[weight,x_data],Tout=tf.float32)
	return output

def train_linear_regression(x_data,y_data):	
	g = tf.get_default_graph()
	weight = tf.constant([0.1,0.2,0.3],dtype = tf.float32)
	#weight1 = tf.constant([0.5],dtype = tf.float32)
	with tf.Session(graph=g,config=config) as sess:
		#weight = tf.Variable(tf.random_uniform([1],-1.,1.))
		sess.run(tf.global_variables_initializer())			
		print(x_data.shape,y_data.shape,type(x_data))
		input = tf.constant(x_data,dtype=tf.float32)
		#print(input.eval())
		z = new_op(input,weight,grad = np_func_grad)
		#print("weight_new:",weight_new.eval())
		print(z.eval())		
		print("GD:",tf.gradients(z,[input,weight])[0].eval(),weight.eval(),x_data)
		#print("weight:",weight.eval(),"x_data:",x_data)
		#print("weight_type:",weight.eval().shape,"x_data_type:",x_data.shape)
'''
