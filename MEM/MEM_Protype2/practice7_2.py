...
def train_linear_regression(x_data,y_data,max_iterate):
    ...
    with tf.Session(graph=graph,config=config) as sess:
        ...
        print(sess.run(y).shape,y_data.shape)
        print(sess.run(tf.gradients(my_loss(y,y_data),[y,y_data])[0]),y.eval().shape,y_data.shape)
        print("GD:",sess.run(tf.gradients(y,[W,x_data,b])[0]),W.eval(),x_data)
